...
# Feature: Clip Upload (import into the archive)

## Purpose
Import an external surveillance clip or snapshot (footage from another NVR, a
phone, a dashcam) into this app's archive on B2 with real-time progress. Live
Frigate footage arrives via the archive worker, not this form.
...
- Side effects: file stored in B2 under `frigate-nvr-archive/imports/{sanitized_filename}`

## Flow
- User drops or selects clips/snapshots in dropzone
...
- API validates content type against the surveillance-media allowlist (mp4/mov/jpg/png/webp)
...
- API uses key: `frigate-nvr-archive/imports/{sanitized_filename}`
- API calls `put_object` to B2
- API extracts snapshot metadata (checksums; image dimensions/EXIF for snapshots)
...
- File type not in allowlist (only clips/snapshots allowed) → API returns 415
