...
# Upload here imports an EXTERNAL clip/snapshot into the archive (e.g. footage
# from another NVR or a phone). Live footage arrives via the archive worker, not
# this form, so the accepted types are surveillance media — clips and snapshots.
ALLOWED_TYPES = {
    "video/mp4",
    "video/quicktime",
    "image/jpeg",
...
def process_upload(
    ...
    # Imported clips/snapshots land under this app's archive prefix so they
    # show up in the Archive Library alongside Frigate-sourced media. B2 buckets
    # are versioned — re-uploading the same key just creates a new version.
    key = f"{settings.archive_prefix}imports/{safe_name}"
