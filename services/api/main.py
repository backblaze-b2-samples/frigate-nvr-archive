...
app = FastAPI(
    title="Frigate NVR Archive API",
    description=(
        "B2 archive + search + playback layer on top of a real Frigate NVR. "
        "Consumes Frigate's HTTP API (events, clips, snapshots, recordings), "
        "ships media + a searchable JSONL detection-event index to Backblaze "
        "B2, and serves clips back via short-lived presigned URLs. Detection "
        "is performed by Frigate, never re-implemented here."
    ),
    version="0.1.0",
    lifespan=lifespan,
)
